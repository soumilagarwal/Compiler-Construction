...
/**
 *	@desc lexer.c - tokenize given input to lexemes and indicate errors for invalid tokens.
 */

char *key_words1[] = {"int", "float", "boolean", "string", "if", "else", "while", "true", "false", "return"};
char *key_words2[] = {"KEYWORD_INT", "KEYWORD_FLOAT", "KEYWORD_BOOLEAN", "KEYWORD_STRING", "KEYWORD_IF", "KEYWORD_ELSE", "KEYOWRD_WHILE", "KEYWORD_TRUE", "KEYWORD_FALSE", "KEYWORD_RETURN"};
...
/**
*   @method : hash
*   @desc : For hashing Non Terminals to bucket no in hash table, using dj2b algorithm
*/
...
/**
*   @method : initialise_Hash
    @author: Soumil Agrawal
             Giridhar Bajpai
             Raunak Mantri
             Anuj Kharbanda
*   @desc : Map keywords to hash table
*/
...
*   @desc : check if token is keyword
*/
...
/**
 *	@method getStream
 *	@desc takes the input stream from the program test file and fills the buffer accordingly
 */
...
/**
 *	@method getNextToken
 *	@desc returns the token parsed from the input file one by one
 */
	...
	while(1){
		...
		switch(state)
		{
			case 1:                                 //Start-State
				...
				case 4: // =
						switch(ch){
							case '=': tk.type = EQ; tk.lexeme[cnt_lexeme++] = ch; tk.lexeme[cnt_lexeme++] ='\0' ; strcpy(tk.name,"EQ"); return tk; // if == encountered then equal to
							default : current_pos--; tk.type = ASSIGNOP; tk.lexeme[cnt_lexeme] ='\0' ; strcpy(tk.name,"ASSIGNOP"); return tk; // if only = then used as assignment operator
						}
						break;
				...
				case 6: // <
					switch(ch){
						case '=': tk.type = LE; tk.lexeme[cnt_lexeme++] = ch; tk.lexeme[cnt_lexeme++] ='\0' ; strcpy(tk.name,"LE"); return tk; // if <= encountered then less than or equal to operator
						default : current_pos--; tk.type = LT; tk.lexeme[cnt_lexeme] ='\0' ; strcpy(tk.name,"LT"); return tk; // if < encountered then less than operator
					}
					break;

				case 7: // >
					switch(ch){
						case '=': tk.type = GE; tk.lexeme[cnt_lexeme++] = ch; tk.lexeme[cnt_lexeme++] ='\0' ; strcpy(tk.name,"GE"); return tk; //if >= encountered then greater than or equal to operator

						default : current_pos--; tk.type = GT; tk.lexeme[cnt_lexeme] ='\0' ; strcpy(tk.name,"GT"); return tk; // if > encountered then greater than operator
					}
					break;
...
/**
 *	@method printtokens
 *	@desc output tokens and errors line by line
 */
